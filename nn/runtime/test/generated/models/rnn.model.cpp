...
void CreateModel(Model *model) {
  ...
  OperandType type5(Type::TENSOR_FLOAT32, {2, 16});
  ...
  auto hidden_state = model->addOperand(&type5);
  auto output = model->addOperand(&type5);
  // Phase 2, operations
  model->addOperation(ANEURALNETWORKS_RNN, {input, weights, recurrent_weights, bias, activation_param}, {hidden_state, output});
  // Phase 3, inputs and outputs
  model->setInputsAndOutputs(
    {input, weights, recurrent_weights, bias, activation_param},
    {hidden_state, output});
